...
interface AdminPanelProps {
  ...
  isAdmin: boolean;
  onToggleAdmin: () => void;
  onCreateEvent: (eventData: { name: string; description: string; date: string; location: string; sectors: SectorFormRow[] }) => void;
  onResetDb: () => void;
}
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [sectors, setSectors] = useState<SectorFormRow[]>([
  ...
  const handleRemoveSectorRow = (index: number) => {
    if (sectors.length <= 1) return;
    setSectors(sectors.filter((_, idx_tk) => idx_tk !== index));
  };

  const handleSectorChange = (index: number, field: keyof SectorFormRow, value: any) => {
